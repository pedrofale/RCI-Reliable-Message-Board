...
// Returns < 0 if there was a communication error. If there wasn't an error, returns the number of 
// registered MSG servers
int COMMMSGSERV_get_server_list(SOCKET* idserv_clientsocket, char *server_list, int server_list_len) {
	...
	tv.tv_sec = TIMEOUT;
	tv.tv_usec = 0;
	if (setsockopt(SOCKET_get_fd(idserv_clientsocket), SOL_SOCKET, SO_RCVTIMEO,&tv,sizeof(tv)) < 0) {
 	...
 	while(strstr(server_list, "SERVERS") == NULL) {
 		if(sendmsg_udp(idserv_clientsocket, msg, sizeof(msg)) == -1) err = -1;
		if(readmsg_udp(idserv_clientsocket, server_list, server_list_len) == -1) err = -2; 		
 	}
...
int COMMMSGSERV_add_smessages(MSGSERV *msgserv, char *string) {
	int err = 0;
	int cnt = 0;
...
int send_messages_to_msgserv(char *buffer, SOCKET *sckt, MSGSERV *msgserv) {
	char resp[MAX_BUFF_SIZE] = "";
	int n, aux;
	int err = 0;
	char lcstr[10];
